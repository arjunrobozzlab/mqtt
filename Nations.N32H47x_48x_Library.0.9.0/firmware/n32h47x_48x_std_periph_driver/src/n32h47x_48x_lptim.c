...
void LPTIM_DeInit(LPTIM_Module* LPTIMx)
{
    ...
    {
        /* Reset LPTIM1 */
        RCC_EnableLPTIMPeriphReset(RCC_LPTIM1_PERIPH_RST);
    } 
    else
    {
        /* Reset LPTIM2 */
        RCC_EnableLPTIMPeriphReset(RCC_LPTIM2_PERIPH_RST);
    }
}
...
*\*\          - LPTIM_TRIG_SOURCE_COMP5
*\*\          - LPTIM_TRIG_SOURCE_COMP6
*\*\          - LPTIM_TRIG_SOURCE_COMP7
...
*\*\          - LPTIM1
*\*\          - LPTIM2
*\*\param   IntStatus (The input parameters must be the following values):
...
uint32_t LPTIM_IsActiveIntStatus(LPTIM_Module* LPTIMx, uint32_t IntStatus)
{
    ...
    /* Read the corresponding bit, set 1 to return 1, otherwise, return 0 */
    Temp = (((READ_BIT(LPTIMx->INTSTS,IntStatus) == IntStatus)? 1UL : 0UL));
...
*\*\          - LPTIM_INPUT1_CONNECT_COMP5
*\*\          - LPTIM_INPUT1_CONNECT_COMP6
*\*\          - LPTIM_INPUT1_CONNECT_COMP7
*\*\return  none
**/
...
*\*\          - LPTIM_INPUT2_CONNECT_COMP5
*\*\          - LPTIM_INPUT2_CONNECT_COMP6
*\*\          - LPTIM_INPUT2_CONNECT_COMP7
*\*\return  none
**/
