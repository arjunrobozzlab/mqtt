...
int main(void)
{
    ...
    USART_ConfigLINBreakDetectLength(USARTx,USART_LINBDL_10B);
    USART_EnableLIN(USARTx,ENABLE);
    
    /* Enable USARTx Receive and Transmit interrupts */
    USART_ConfigInt(USARTx, USART_INT_RXDNE,ENABLE);
    USART_ConfigInt(USARTx, USART_INT_LINBD,ENABLE);
...
{
    /* Enable GPIO clock */
    GPIO_AHBClkCmd(USARTx_GPIO_CLK,ENABLE);
