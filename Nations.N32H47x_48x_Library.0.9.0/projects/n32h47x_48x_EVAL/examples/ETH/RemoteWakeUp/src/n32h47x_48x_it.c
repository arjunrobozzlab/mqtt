...
/** N32H47X_48X Peripherals Interrupt Handlers, interrupt handler's name please refer 
    to the startup file (startup_n32h487.s) **/
...
/**
*\*\name    ETH_WKUP_IRQHandler.
*\*\fun     This function handles Ethernet wakeup through EXTI Line 27 Handler.
*\*\param   none
*\*\return  none 
**/
void ETH_WKUP_IRQHandler(void)
{
    ...
    if (EXTI_GetITStatus(EXTI_LINE27) != RESET)
    {
        /* Clear the EXTI line pending bit */
        EXTI_ClrITPendBit(EXTI_LINE27);
        
        if (ETH_GetMACInterruptStatus(ETH, ETH_MAC_INT_FLAG_PMT) != RESET)
        {
            ETH_ClearPMTFlag(ETH, ETH_PMT_FLAG_WUPR);
        }
    }
