...
/**
*\*\name    LPTIM1_WKUP_IRQHandler.
*\*\fun     This function handles LPTIM1 wakeup Handler.
*\*\param   none
*\*\return  none 
**/
void LPTIM1_WKUP_IRQHandler(void)
{
    if (LPTIM_IsActiveIntStatus(LPTIM1, LPTIM_COMP_MATCH_STATUS) != RESET)
    {
        /* Toggle LED2 */
        LedBlink(LED2_PORT, LED2_PIN);
        
        LPTIM_ClearIntFlag(LPTIM1, LPTIM_COMP_MATCH_FLAG);
        EXTI_ClrITPendBit(EXTI_LINE25);
    }
}
