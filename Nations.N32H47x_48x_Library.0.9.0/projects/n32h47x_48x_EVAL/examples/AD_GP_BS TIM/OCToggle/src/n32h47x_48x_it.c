...
{
    /* TIMx_CH1 toggling */
    if (TIM_GetIntStatus(TIMx, TIM_INT_CC1) != RESET)
    {
        TIM_ClrIntPendingBit(TIMx, TIM_INT_CC1);
        capture = TIM_GetCap1(TIMx);
        TIM_SetCmp1(TIMx, capture + CCR1_Val);
    }

    /* TIMx_CH2 toggling */
    if (TIM_GetIntStatus(TIMx, TIM_INT_CC2) != RESET)
    {
        TIM_ClrIntPendingBit(TIMx, TIM_INT_CC2);
        capture = TIM_GetCap2(TIMx);
        TIM_SetCmp2(TIMx, capture + CCR2_Val);
    }

    /* TIMx_CH3 toggling */
    if (TIM_GetIntStatus(TIMx, TIM_INT_CC3) != RESET)
    {
        TIM_ClrIntPendingBit(TIMx, TIM_INT_CC3);
        capture = TIM_GetCap3(TIMx);
        TIM_SetCmp3(TIMx, capture + CCR3_Val);
    }

    /* TIM3_CH4 toggling */
    if (TIM_GetIntStatus(TIMx, TIM_INT_CC4) != RESET)
    {
        TIM_ClrIntPendingBit(TIMx, TIM_INT_CC4);
        capture = TIM_GetCap4(TIMx);
        TIM_SetCmp4(TIMx, capture + CCR4_Val);
    }
}
